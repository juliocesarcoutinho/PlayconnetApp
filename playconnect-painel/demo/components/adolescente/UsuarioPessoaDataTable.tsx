...
const UsuarioPessoaDataTable = () => {
  ...
  const [selectedUsuario, setSelectedUsuario] = useState<UsuarioPessoa | null>(
    null
  );
  ...
  useEffect(() => {
    const fetchUsuarios = async () => {
      setLoading(true);
      try {
        const fetchedUsuarios: Demo.UsuarioPessoa[] =
          await UsuarioService.getUsuarioPessoa();
        const usuariosComId: UsuarioPessoa[] = fetchedUsuarios.map(
          (usuario) => ({
            ...usuario,
            id: usuario.id ?? 0,
            pessoa: {
  ...
  const saveUsuario = async () => {
    if (selectedUsuario) {
      try {
        const response = await UsuarioService.editUsuarioPessoa(
          selectedUsuario
        );
        if (response.success) {
          toast.current?.show({
  ...
  return (
    <div className="card">
      ...
      >
        <Column
          ...
          body={(rowData) => (
            <div
              ...
              onKeyUp={(e) => e.key === "Enter" && onCellClick(rowData)}
        ...
        />
        <Column
          field="pessoa?.endereco?.logradouro"
          header="Logradouro"
          sortable
          body={(rowData) => (
            <div

...
const LoginPage = () => {
    ...
    LoginService.login(email, password).then((response) => {
        if (response.success) {
            router.push('../pages/dashboard'); // Redirect to the main page
        } else {
            toast.current?.show({
                severity: 'error',
                summary: 'Erro',
    ...
    return (
        <BlockUI blocked={isLoading} fullScreen>
            ...
            {isLoading && (
                <div className="flex align-items-center justify-content-center" style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 10000 }}>
                    <ProgressSpinner />
                </div>
            )}
        </BlockUI>
